...
For queries on large wikis (100+ pages), also run a `search_files` for the topic before creating anything new.
...
|---|---|---|
| URL (article) | `web_extract` | `~/wiki/llm-wiki/raw/articles/<slug>.md` |
| PDF / arxiv | `web_extract` | `~/wiki/llm-wiki/raw/papers/<slug>.md` |
| Pasted text | `write_file` | appropriate `raw/` subdir |
...
- `read_file ~/wiki/llm-wiki/index.md`
- `search_files` for entities/concepts across the wiki:
  ```
  search_files(pattern: "LoRA", path: "/Users/charlesqin/wiki/llm-wiki", glob: "*.md", output_mode: "files_with_matches")
  ```
...
1. Orient: `read_file ~/wiki/llm-wiki/SCHEMA.md`, `~/wiki/llm-wiki/index.md`, and every file in the target subtree.
...
Hermes tools used by this skill:

| Operation | Tool | Notes |
|---|---|---|
| Read a wiki page or SCHEMA | `read_file` | Use `offset`/`limit` for long files |
| Search wiki content | `search_files` | `path: "/Users/charlesqin/wiki/llm-wiki"`, `glob: "*.md"` |
| List all pages | `search_files` | `pattern: "**/*.md"` |
| Fetch URL/PDF | `web_extract` | Saves markdown from the web |
| Write new page | `write_file` | Only for new files |
| Update existing page | `apply_patch` | Always bump `updated` date |
| Run lint script | `execute_code` | Python script above |
...
- **Don't cross-contaminate with Hermes session state** — research goes in wiki, user preferences in memory
